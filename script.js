// Code by Khushboo Aswani
// The below code makes a favourites meal array if it's does not exist in local storage
//The getItem() method is used to retrieve the storage object which is specified by the user.
if (localStorage.getItem("favouritesList") == null) {
    localStorage.setItem("favouritesList", JSON.stringify([]));
}

// The below code fetches meals from api and returns it back
...
// The below code show's all the meals cards in the Main screen acording to the search input value entered by the user
...
//The below code shows full meal details including it's description and YouTube Video
...
// The below code shows all the Favourites meals added by the user in the Favourites body Section
...
//The below code adds and remove meals accoring to the selection by the user in the Favourites list Section 
function addRemoveToFavList(id) {
    ...
    if (contain) {
        ...
        arr.splice(number, 1);
        alert("Meal is removed from the favourites list");
    } else {
        arr.push(id);
        alert("Meal is added to the favourites list");
    }
